...
def test_export_on_a_freshly_migrated_instance_reflects_seed_state():
    async def run(pool):
        ...
        assert bundle["schema_version"] == 19
        ...
        assert bundle["settings"] == {"auto_assign_default_vehicle": False}
    ...
    async def run(pool):
        async with pool.connection() as conn:
            ...
            place_cur = await conn.execute(
                ...
                "VALUES ('Office', 'work', ST_SetSRID(ST_MakePoint(-122.3, 47.6), 4326)::geography, 100) "
                "RETURNING id",
            )
            ...
            trip_cur = await conn.execute(
                ...
                "VALUES ('manual', 'manual', %s, %s, 1609.344, 'business', 'Client visit', "
...
def test_export_refuses_to_emit_a_non_finite_value():
    ...
    async def run(pool):
        async with pool.connection() as conn:
            await conn.execute(
                "INSERT INTO trips (device, source, started_at, ended_at, distance_m, category) "
                "VALUES ('phone1', 'manual', %s, %s, 'Infinity'::real, 'unclassified')",
                (NOW, NOW),
            )
