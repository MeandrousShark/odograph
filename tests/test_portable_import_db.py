...
@pytest.mark.parametrize("schema_version", [26, 27, 28, 29, 30, 31, 32])
def test_format_3_bundles_import_into_schema_32(schema_version):
    ...
    authentication. Format-3 exports from schema 26 through 32 import."""
    async def run(pool):
        transport = httpx.ASGITransport(app=_bare_app(pool))
        async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
            csrf = await _csrf(client)
